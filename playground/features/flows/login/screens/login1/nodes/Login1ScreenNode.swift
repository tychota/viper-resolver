...
extension Login1ScreenNode {
    override func layoutSpecThatFits(_ constrainedSize: ASSizeRange) -> ASLayoutSpec {
        let frontSpec = ASStackLayoutSpec(direction: .vertical,
                                          spacing: 0,
...
extension Login1ScreenNode {
    override func didLoad() {
        homeButton.addTarget(self, action: #selector(homeButtonPressed), forControlEvents: .touchUpInside)
    }
    
    @objc
    func homeButtonPressed() {
        presenter.handleHomeButtonPressed()
    }
}
